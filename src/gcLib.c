...
void gc_UnloadCamera(Camera2D* cam2D) { free(cam2D); }
...
void gc_PlaySound(Sound* sound) { PlaySound(*sound); }
bool gc_IsSoundPlaying(Sound* sound) { return IsSoundPlaying(*sound); }
...
bool gc_CheckCollisionCircles( float c1X, float c1Y, float radius1, float c2X, float c2Y, float radius2 ) {
		return CheckCollisionCircles(
			(Vector2){c1X, c1Y}, radius1, 
			(Vector2){c2X, c2Y}, radius2
		);
	}
bool gc_CheckCollisionCircleRec( float CenterX, float CenterY, float radius, float RecX, float RecY, float RecWidth, float RecHeight ) {
		return CheckCollisionCircleRec(
			(Vector2){CenterX, CenterY}, radius, 
			(Rectangle){RecX, RecY, RecWidth, RecHeight}
		);
	}
bool gc_CheckCollisionMousetoRec( float PointX, float PointY, float RecX, float RecY, float RecWidth, float RecHeight ) {
